...
The same shape can also be built by hand, which is a good illustration of how `lazy` enables recursive schemas (note that `jsonValue` is implemented directly and reports different issues than this hand-rolled composition):
