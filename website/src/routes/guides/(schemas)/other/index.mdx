...
Another practical use case for `lazy` is a schema for all possible `JSON` values, meaning strings, finite numbers, booleans, `null`, and objects or arrays that recursively contain only these types. Valibot ships this as a built-in schema, <Link href="/api/jsonValue/">`jsonValue`</Link>, so you don't need to write it yourself.
...
The same shape can also be built by hand, which is a good illustration of how `lazy` enables recursive schemas. Note that `jsonValue` is implemented directly rather than composed like this, and behaves differently in a few ways: it rejects non-finite numbers such as `Infinity` and `-Infinity`; it detects circular references and rejects them with an issue instead of overflowing the call stack; it rejects values that `JSON.stringify` would otherwise serialize, such as `Date` instances, other class instances, and sparse arrays (whose holes `JSON.stringify` turns into `null`); and it reports different issues:
