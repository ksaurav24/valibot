...
Another practical use case for `lazy` is a schema for all possible `JSON` values. These are all values that can be serialized and deserialized using `JSON.stringify()` and `JSON.parse()`. Valibot ships this as a built-in schema, <Link href="/api/jsonValue/">`jsonValue`</Link>, so you don't need to write it yourself.
...
The same shape can also be built by hand, which is a good illustration of how `lazy` enables recursive schemas (note that `jsonValue` is implemented directly and, unlike this hand-rolled composition, rejects non-finite numbers such as `Infinity` and `-Infinity`, in addition to reporting different issues):
